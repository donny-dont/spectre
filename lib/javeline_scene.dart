#library('javeline_scene');
#import('dart:html');
#import('dart:math', prefix:'Math');
#import('package:DartVectorMath/vector_math_html.dart');
#import('handle.dart');
#import('spectre.dart');
#import('spectre_scene.dart');
#import('skybox.dart');
#import('hfluid.dart');
#source('javeline_scene/loader.dart');
#source('javeline_scene/scene_child.dart');
#source('javeline_scene/scene.dart');
#source('javeline_scene/mesh.dart');
#source('javeline_scene/uniformset.dart');
#source('javeline_scene/material.dart');
#source('javeline_scene/material_instance.dart');
#source('javeline_scene/model.dart');
#source('javeline_scene/transform_controller.dart');