...
class PosedSkeleton {
  ...
  final List<Float32x4List> globalTransformsSIMD;
  final List<Float32x4List> skinningTransformsSIMD;
  PosedSkeleton(this.skeleton, int length) :
    ...
    globalTransformsSIMD = new List<Float32x4List>(length),
    skinningTransformsSIMD = new List<Float32x4List>(length){
    for (int i = 0; i < length; i++) {
      globalTransforms[i] = new Float32List(16);
      skinningTransforms[i] = new Float32List(16);
      
      globalTransformsSIMD[i] = new Float32x4List.view(globalTransforms[i].buffer);
      skinningTransformsSIMD[i] = new Float32x4List.view(skinningTransforms[i].buffer);
    }
  }
}
